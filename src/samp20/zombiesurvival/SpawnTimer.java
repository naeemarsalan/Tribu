...
public class SpawnTimer implements Runnable {
	...
	public void run() {
		if(plugin.getSpawner().getTotal() < totalSpawn && plugin.running() && plugin.getAliveCount() > 0) {
	...
	public void Start() {
		taskID = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, this, Constants.SpawnDelay, Constants.SpawnDelay);
	}
